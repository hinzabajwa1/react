...
function App() {
  ...
  //Actually the initial state has color olive and the reload technology reset the state ,so its the olive color
  //onClick={()=>setColor("red")} the onclick method in react expects a function reference .You cant directly pass the parameter  inside the function,instead you need to pass it as a reference or use arrow functions syntax
  return (
    <>
     ...
     >
      <div className="fixed  flex flex-wrap justify-center bottom-12 inset-x-0 px-2">
        <div className="flex flex-wrap justify-center gap-3 shadow-lg bg-white px-3 py-2 rounded-3xl">
          ...
          <button onClick={()=>setColor("purple")} className="outline-none px-4 py-1 rounded-full text-white" style={{backgroundColor : "purple"}}>Red</button>
          <button onClick={()=>setColor("lavender")} className="outline-none px-4 py-1 rounded-full text-black" style={{backgroundColor : "lavender"}}>Lavender</button>
